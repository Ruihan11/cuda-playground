/*
nvcc -g -G -O0 -o temp/atten_debug kernels/atten/v2_fa2_atten.cu
*/
...
__global__ void fa2_kernel(const float *Q, const float *K, const float *V,
                           float *output, const int64_t batch, const int heads,
                           const int64_t seq, const int dimension,
                           const float scale) {

  int64_t i = blockIdx.y; // #seq - #rows
  ...
  for (int64_t j = 0; j < seq; j++) {
    const float *Qi = Q + (int64_t)b * stride_b + (int64_t)h * stride_h +
                      (int64_t)i * stride_s;
    const float *Ki = K + (int64_t)b * stride_b + (int64_t)h * stride_h +
                      (int64_t)j * stride_s;
    const float *Vi = V + (int64_t)b * stride_b + (int64_t)h * stride_h +
                      (int64_t)j * stride_s;
    ...
    for (int64_t k = 0; k < dimension; k++)
      o[k] = expf(m - m_new) * o[k] + expf(acc - m_new) * Vi[k];

    m = m_new;
    l = l_new;
  }

  float *Oi = output + (int64_t)b * stride_b + (int64_t)h * stride_h +
              (int64_t)i * stride_s;
...
static void run_kernels(const float *d_q, const float *d_k, const float *d_v,
                        float *d_r, int64_t batch, int heads, int seq, int d) {
